...
const knownPkmImplementations: {
  [version: number]: { new (pkx: Uint8Array, box: number, slot: number, ghost: boolean): PkBase };
} = {};
...
export function registerPkmImpl(
  generation: number,
  impl: { new (pkx: Uint8Array, box: number, slot: number, ghost: boolean): PkBase }
) {
...
export default class PkBase {
  ...
  constructor(pkx: Uint8Array, box: number, slot: number, isGhost: boolean) {
    ...
    this.eggDate =
      (pkx[0xd1] | pkx[0xd2] | pkx[0xd3]) != 0 ? [pkx[0xd1] + 2000, pkx[0xd2] - 1, pkx[0xd3]] : [2000, 0, 1];
    this.metDate =
      (pkx[0xd4] | pkx[0xd5] | pkx[0xd6]) != 0 ? [pkx[0xd4] + 2000, pkx[0xd5] - 1, pkx[0xd6]] : [2000, 0, 1];
    // 0xD7 - unused
  ...
  static deshuffle(pkx: Uint8Array, sv: number) {
    ...
    for (var b = 0; b < 4; b++) util.copy(pkx, 8 + 56 * shuffle[b], ekx, 8 + 56 * b, 56);
  ...
  static getDloc(ec: number) {
    return [3, 2, 3, 2, 1, 1, 3, 2, 3, 2, 1, 1, 3, 2, 3, 2, 1, 1, 0, 0, 0, 0, 0, 0][((ec & 0x3e000) >> 0xd) % 24];
  }
}
